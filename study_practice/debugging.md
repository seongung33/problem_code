## 디버깅
오류 내역은 없이 빠지거나 잘못 표기되었을 것 같은거 위주로 정리
...
## 부족한 점 
일대다, 다대다, Auth 등에 대한 설명이 꽤 부족함. 일대다의 on_delete나 비밀번호 변경 등에 쓰이는 from django.contrib.auth import logout as auth_logout 와 같은 것들은 추가 학습 필요 이 외에도 @로 쓰는 데코레이터(로그인 해야만 좋아요 가능 같은 것) 추가학습 하면 좋을 듯 합니다.  

위에서 언급한 것 이외에도 말 안한 것 많을듯 하니까 강사님 코드도 좀 보기 절대절대 이거에 의존하지 않기